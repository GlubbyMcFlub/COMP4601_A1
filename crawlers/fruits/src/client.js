...
const c = new Crawler({
	...
	callback: async function (error, res, done) {
		if (error) {
			...
			wordFrequencies = Object.entries(wordFrequencies).sort((a, b) => b[1] - a[1]);
			...
			try {
				...
				if (response.status === 200) {
					...
					outgoingLinks.forEach(async (link) => {
						let outgoingLinkGetQuery = "http://localhost:5000/links/?link=" + link;
						const outgoingLinkGetResponse = await fetch(
							outgoingLinkGetQuery,
							{
								method: "GET",
								headers: {
									"Content-Type": "application/json",
						...
						const outgoingLinkPostResponse = await fetch(
						...
						if (outgoingLinkPostResponse.status === 200 && outgoingLinkGetResponse.status === 404) {
								c.queue(link);
						}
						else if(outgoingLinkPostResponse.status != 200){
							console.error(
